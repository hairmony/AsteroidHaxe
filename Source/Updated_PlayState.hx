...
import Enemy2; // NEW: zig-zag enemy variant
...
class PlayState extends FlxState
{
    ...
    function collideProjectile(object1:FlxObject, object2:FlxObject):Void
    {
        ...
        else if (Std.isOfType(object1, Enemy) || Std.isOfType(object1, Enemy2)) // UPDATED: count Enemy2 as well
        {
    ...
    function startWave():Void
    {
        ...
        if (enemiesToSpawn > 0) // for some reason FlxTimer loops inf times if the value is 0
        {
            new FlxTimer().start(0.5, function(timer:FlxTimer)
            {
                ...
                {
                    // NEW: randomly spawn Enemy or Enemy2
                    var e:FlxSprite;
    ...
    function enemyShot(timer:FlxTimer): Void {
        ...
        for(i in enemy){
            // NEW: use FlxSprite so it works for both Enemy and Enemy2
            var temp:FlxSprite = cast(i, FlxSprite); //create a temporary sprite based off the inspected FlxGroup element
